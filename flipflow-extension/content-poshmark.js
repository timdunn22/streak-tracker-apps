...
(() => {
  ...
  async function shareAllListings() {
    ...
    for (let i = 0; i < maxShares; i++) {
      ...
      /* human-like delay 2-10 seconds with occasional longer pauses */
      let delay = FlipFlow.randomDelay(2000, 10000);
      if ((i + 1) % FlipFlow.randomDelay(8, 15) === 0) {
        /* every 8-15 shares, take a longer "browsing break" */
        delay = FlipFlow.randomDelay(15000, 30000);
        progress.setStatus(`Pausing naturally... (${i + 1} of ${maxShares})`);
      }
