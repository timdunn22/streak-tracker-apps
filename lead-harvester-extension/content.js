...
(function () {
  ...
  async function exportToColdFlow() {
    const leadsWithEmails = scrapedLeads.filter((l) => l.email);
    if (leadsWithEmails.length === 0) {
      setStatus('No leads with emails. Try "Visit Websites" first.', 'info');
      return;
    }

    const contacts = leadsWithEmails.map((l) => ({
      name: l.name || '',
      email: l.email,
      company: l.name || '',
      phone: l.phone || '',
    ...
    // Save to shared storage key that ColdFlow can read
    await storageSet({ coldflow_imported_contacts: contacts });
    setStatus(
      `Exported ${contacts.length} contacts for ColdFlow. Open ColdFlow to import.`,
      'success'
    );
  }
    ...
    panel.innerHTML = `
      ...
      <div class="lh-actions">
        ...
        <div class="lh-actions-row">
          ...
          <button class="lh-btn lh-btn-secondary" id="lh-export-coldflow" title="Save leads for ColdFlow email sequences">
            Send to ColdFlow
          </button>
        </div>
        <div class="lh-actions-row">
  ...
  function wireEvents() {
    ...
    const coldflowBtn = document.getElementById('lh-export-coldflow');
    ...
    if (coldflowBtn) {
      coldflowBtn.addEventListener('click', exportToColdFlow);
    }
    if (clearBtn) {
